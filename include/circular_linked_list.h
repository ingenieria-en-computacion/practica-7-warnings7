...
#define DECLARE_LINKED_LIST(TYPE) \
    ...
    \
    List_##TYPE* list_##TYPE##_create(bool circular); \
...
#define IMPLEMENT_LINKED_LIST(TYPE) \
    ...
    bool list_##TYPE##_insert(List_##TYPE* list, TYPE data, size_t pos) { \
        ...
        if (pos == 0) { \
            ...
            } else if (list->is_circular) { \
                list->tail->next = list->head; \
            } \
        } else if (pos == list->length) { \
            new_node->next = list->is_circular ? list->head : NULL; \
            if (list->tail) list->tail->next = new_node; \
            list->tail = new_node; \
            if (list->length == 0) list->head = new_node; \
        } else { \
            Node_##TYPE* current = list->head; \
    ...
    void list_##TYPE##_print(const List_##TYPE* list, void (*print_fn)(TYPE)) { \
        ...
        printf("["); \
        Node_##TYPE* current = list->head; \
        for (size_t i = 0; i < list->length && current; ++i) { \
